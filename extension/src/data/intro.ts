...
/** The claimed walkthrough's note, waiting for the Extension Assistant session the board is about to start. */
...
/**
 * Whether entering the board should start the Extension Assistant chat itself. The marker is the first-run test:
...
function closingLines(roster: Persona[]): string[] {
	...
	if (manager) {
		...
		return [
			`Close in two short lines: this project already has a ${manager.title}${name ? `, who goes by ${name}` : ''}, who owns showing and running the project team. Tell them to click that card on Team, and stop.`,
			`Do not list or explain the roster. Do not offer to create a Project Manager, and do not edit any files. If they ask who owns what or a role is missing, offer to hand that persona a note rather than answering yourself.`,
		];
	}
	const offer = roster.length
		? `Close by offering one thing, in two short lines: a Project Manager, who will show them the project team — mapping, delegation, and the roster — and once they have that persona they can send you away. This project already has other personas; do not list or explain them. Ask whether to add the Project Manager, then stop and wait. Do not inventory the project. Do not edit any files on this turn.`
		: `Close by offering one thing, in two short lines: a Project Manager, who will show them the project team — mapping, delegation, and the roster — and once they have that persona they can send you away. Ask whether to add it, then stop and wait. Do not inventory the project. Do not edit any files on this turn.`;
	return [
		offer,
...
export function introNote(root?: string): string {
	...
	return [
		...
		`Open with exactly this line, then finish the same paragraph with one sentence on what you do — extension walkthrough and support, not the project team: "I'm the Extension Assistant — I walk you through this extension and help when something in it breaks." Do not explain this project's personas; that is the Project Manager's job and you offer that card at the end. Do not name internal persona ids and do not say which persona you are wearing.`,
		...
		`Then one sentence on what Context Manager does about it: it puts that existing context on Team and Agent, visible and editable in place.`,
		`Then four markdown bullets, a few words each, no labels with colons: click a figure on Team to start a chat; on Agent see and adjust that persona's context; hand a note to another session or subagent; edit the context markdown without leaving Team or Agent.`,
		...closingLines(roster),
	].join('\n');
}
 ...
 * user starts themselves would otherwise make some implementer deliver the Extension Assistant introduction.
 */

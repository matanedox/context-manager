...
assert.equal(
  createContextFile(workflowRoot, { kind: "workflow", name: "Ship It", description: "release run" }),
...
assert.equal(emptyCtx.available.some((item) => item.type === "skill"), false, "empty skills stay empty");
...
assert.deepEqual(
  decoratedCategorized.categoryIds,
  ["my-picks"],
  "new context keeps its custom-tab membership"
);
...
assert.equal(
  globalRuleFile("global:rules/review-bar.mdc", cursorHome),
  fs.realpathSync(createdGlobal)
);
assert.equal(
  globalRuleFile("global:../outside.md", cursorHome),
  null,
  "global rule ids cannot escape ~/.cursor"
);
assert.equal(
...
const betaTabs = payloadFor(sampleState, false, roleMap, "conv-a", charterCtx, Date.now())
  .contextTabs;
...
fs.writeFileSync(
  path.join(sectionedRoot, ".cursor", "personas", "project-manager.md"),
  sectionedPersona
);
...
fs.writeFileSync(
  path.join(charterRoot, ".cursor", "personas", "project-manager.md"),
  sectionedPersona
);
...
assert.deepEqual(
  fs.readdirSync(emptyWorkspace),
  [],
  "reading an unconfigured workspace creates no files in it"
);
...
assert.deepEqual(
  JSON.parse(
    fs.readFileSync(path.join(fallbackRoot, ".cursor", "agent-viz", "personas.json"), "utf8")
  ).map((entry) => entry.id),
  ["security"],
...
assert.equal(
  createPersona(addCharterRoot, { role: "Security", name: "SecOps", description: "reviews secrets" }),
  ".cursor/personas/team-roles.md"
);
assert.ok(
  fs.readFileSync(path.join(addCharterRoot, ".cursor", "personas", "team-roles.md"), "utf8").includes(
    "## security"
  ),
